---
description: Java 命令行客户端完整使用示例——轨道倾角改变场景，涵盖连接、新建场景、构建机动规划序列、运行仿真、保存及断开连接的全流程命令。
---
...
以下以**轨道倾角改变场景**为例，演示通过命令行客户端操控 ATK 的完整流程：将卫星从初始近地轨道经三次机动转移到地球同步轨道，同时将轨道倾角从 28° 降为 0°。

::: tip 案例详解
本示例侧重展示命令行客户端的使用流程。更多的案例原理、参数含义及分步截图请参考 [二次开发案例教程](/02-案例教程/8-二次开发案例/)。
:::

## 完整命令

完整命令按照 **连接 → 创建场景 → 设置初始轨道 → 构建三组机动瞄准序列 → 最终预报 → 运行** 的主线编排，各步骤通过注释分隔。

::: details **完整命令**（点击展开可一键复制）
...
// 设置卫星轨道预报器类型为机动规划（第一级目录默认存在 Initial_State 和 Propagate 两个段）
...
// ============================================================
// 6. 第一组：Propagate + Target_Sequence — 抬升远地点至 GEO 高度
// ============================================================
...
// ============================================================
// 7. 第二组：Propagate1 + Target_Sequence1 — 在远地点圆化轨道（偏心率 → 0）
// ============================================================
...
// ============================================================
// 8. 第三组：Propagate2 + Target_Sequence2 — 在升交点改变倾角（倾角 → 0°，偏心率 → 0）
// ============================================================
...
// ============================================================
// 9. 最终预报段 Propagate3 — 停止条件为飞行时间 129600 秒
// ============================================================
...
::: warning 前置准备
- 使用前请确保已完成 [环境配置](./1-简介与配置.md#配置步骤)：已安装 Java 环境
- 运行前需先打开 ATK 软件
- 本示例中有 `New` 命令新建场景，而 ATK CONNECT 模式仅支持加载一个场景，因此**不要打开任何已有场景**；若已打开请先关闭
:::

1. 在 <PathCopy :entry="javaDir" /> 目录下打开命令行（cmd），启动客户端：
...
2. 看到 `开始等待连接指令输入` 提示后，展开上方命令列表，从上到下逐行复制到命令行窗口，按 <kbd>Enter</kbd> 执行。
