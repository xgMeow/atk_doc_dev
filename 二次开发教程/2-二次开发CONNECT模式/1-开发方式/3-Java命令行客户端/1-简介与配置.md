...
启动后在命令行中看到 `开始等待连接指令输入` 提示后，直接输入 [Connect 命令](/二次开发教程/2-二次开发CONNECT模式/2-命令参考/1-命令语法约定.md)，按 <kbd>Enter</kbd> 执行。使用方式与 [Python 客户端工具](/二次开发教程/2-二次开发CONNECT模式/1-开发方式/2-Python客户端工具/1-简介与配置.md) 的命令窗口类似，共享相同的通信协议和命令格式。
