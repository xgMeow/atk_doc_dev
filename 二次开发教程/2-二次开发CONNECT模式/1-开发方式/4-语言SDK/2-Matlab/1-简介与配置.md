...
作为 CONNECT 模式的核心开发方式之一，Matlab SDK 与 ATK 内置的 [Python 客户端工具](/二次开发教程/2-二次开发CONNECT模式/1-开发方式/2-Python客户端工具/1-简介与配置.md) 共享相同的通信协议和命令格式，区别在于 Matlab SDK 运行在用户自建的 Matlab 环境中，适合需要将 ATK 功能集成到 Matlab 项目、或需要利用 Matlab 矩阵计算与可视化能力的场景。
...
ATK 的 Matlab 通信库随安装包一同提供，位于安装包目录下的 `IntegratingWithATK\connect\Matlab` 中，按平台提供了两个版本：

- **Windows**：<PathCopy :entry="matlabDir" /> 文件夹，基于 Matlab R2015b
- **Linux**：<PathCopy :entry="linuxDir" /> 文件夹，适用于麒麟（Kylin）操作系统

两个版本的库文件功能一致。以下以 Windows 版本为例说明文件构成：
...
根据使用的平台，将对应文件夹路径添加到 Matlab 搜索路径：

- **Windows**：将 <PathCopy :entry="matlabDir" label="Win_2015b" /> 加入搜索路径

```matlab
addpath('<ATK安装目录>\IntegratingWithATK\connect\Matlab\Win_2015b')
```

- **Linux**：将 <PathCopy :entry="linuxDir" /> 加入搜索路径

```matlab
addpath('<ATK安装目录>/IntegratingWithATK/connect/Matlab/Linux_2021b')
```
...
const linuxDir = { path: 'IntegratingWithATK\\connect\\Matlab\\Linux_2021b', name: 'Linux_2021b' };
