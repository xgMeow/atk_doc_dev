...
作为 CONNECT 模式的核心开发方式之一，Python SDK 与 ATK 内置的 [Python 客户端工具](/二次开发教程/2-二次开发CONNECT模式/1-开发方式/2-Python客户端工具/1-简介与配置.md) 共享相同的通信协议和命令格式，区别在于 Python SDK 运行在用户自建的 Python 环境中，适合需要将 ATK 功能集成到自有项目、或需要利用外部 Python 生态（如 numpy、matplotlib 等第三方库）的场景。
