---
description: Matlab SDK 完整使用示例——轨道倾角改变场景，涵盖连接、新建场景、构建三组机动瞄准序列、运行仿真、保存及断开连接的全流程代码。
---
...
以下以**轨道倾角改变场景**为例，演示 Matlab SDK 操控 ATK 的完整流程：将卫星从初始近地轨道（sma = 6570 km, inc = 28°）经三次轨道机动转移到地球同步轨道，同时将轨道倾角从 28° 降为 0°。

::: tip 案例详解
本示例侧重展示 Matlab SDK 的使用流程。更多的案例原理、参数含义及分步截图请参考 [二次开发案例教程](../../../../../02-案例教程/8-二次开发案例/)。
:::
...
::: warning 前置准备
- 使用前请确保已完成 [环境配置](./1-简介与配置.md#配置步骤)：将 Matlab 通信库路径添加到 `addpath`
- 运行前需先打开 ATK 软件
- 本示例代码中有 `New` 命令新建场景，而 ATK CONNECT 模式仅支持加载一个场景，因此**不要打开任何已有场景**；若已打开请先关闭
:::

将上方完整代码复制保存为 `.m` 文件（如 `inclination_change.m`），在 Matlab 中打开并点击 <kbd>运行</kbd>。
...
![场景仿真界面](../../../../../../zh/二次开发教程/2-二次开发CONNECT模式/1-开发方式/4-语言SDK/2-Matlab/media/3-完整示例/image0108-17.png)

可在 ATK 中右键点击卫星对象，选择**属性**查看代码设置的各项参数是否正确生效：

![卫星属性界面](../../../../../../zh/二次开发教程/2-二次开发CONNECT模式/1-开发方式/4-语言SDK/2-Matlab/media/3-完整示例/image0108-13.png)
