---
description: Python 客户端工具完整使用示例——轨道倾角改变场景，涵盖连接、新建场景、构建机动规划序列、运行仿真、保存及断开连接的全流程代码。
---
...
以下以**轨道倾角改变场景**为例，演示 Python 客户端工具操控 ATK 的完整流程：将卫星从初始近地轨道经三次机动转移到地球同步轨道，同时将轨道倾角从 28° 降为 0°。

::: tip 案例详解
本示例侧重展示 Python 客户端工具的使用流程。更多的案例原理、参数含义及分步截图请参考 [二次开发案例教程](../../../../02-案例教程/8-二次开发案例/)。
:::

## 完整代码

完整代码按照 **连接 → 创建场景 → 设置初始轨道 → 构建三组机动瞄准序列 → 最终预报 → 运行** 的主线编排，各步骤通过注释分隔，便于逐段理解和复用。

::: details **完整代码**（三种方式通用，点击展开可一键复制）
...
以下三种方式均可运行下方的完整代码，任选其一即可：

::: warning 前置准备
- 运行前需先打开 ATK 软件
- 本实例代码中有 `New` 命令新建场景，而 ATK CONNECT 模式仅支持加载一个场景，因此**不要打开任何已有场景**；若已打开请先关闭
:::

1. **脚本窗口（推荐）**：展开上方代码块，全部复制后粘贴到脚本窗口，点击 <kbd>Run</kbd> 一键运行。

   ![脚本窗口运行](../../../../../zh/二次开发教程/2-二次开发CONNECT模式/1-开发方式/2-Python客户端工具/media/3-完整示例/image0108-19.png)

2. **命令窗口**：展开上方代码块，从上到下逐段复制到命令窗口，按 <kbd>Enter</kbd> 逐行执行。

   ![命令窗口输入](../../../../../zh/二次开发教程/2-二次开发CONNECT模式/1-开发方式/2-Python客户端工具/media/3-完整示例/image0108-5.png)

3. **打开脚本文件**：展开上方代码块，全部复制后保存为 `.atks` 文件，点击 <kbd>Open</kbd> 打开 → <kbd>Run</kbd> 运行。

   ![打开脚本文件](../../../../../zh/二次开发教程/2-二次开发CONNECT模式/1-开发方式/2-Python客户端工具/media/3-完整示例/image0108-24.png)

## 运行结果

运行成功后，ATK 三维窗口将显示卫星的机动规划轨迹。卫星从初始近地轨道出发，经过三次轨道机动后到达地球同步轨道，轨道倾角从 28° 降为 0°。

![场景仿真界面](../../../../../zh/二次开发教程/2-二次开发CONNECT模式/1-开发方式/2-Python客户端工具/media/3-完整示例/image0108-17.png)
