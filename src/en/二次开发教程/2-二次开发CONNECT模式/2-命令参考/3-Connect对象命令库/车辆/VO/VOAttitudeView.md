...
Set the ground vehicle attitude sphere properties.
...
If `AttitudeViewOption` is `Sphere`, the following `SphereOption` properties are available:
...
If `SphereOption` is `Frame`, the following `{CrdnDefOptions}` properties can be set:
...
> Note: If `CBName` and `ObjectPath` are not entered, `Name` defaults to the current object's Axes.

If `AttitudeViewOption` is `Projection`, the following `ProjectionOption` properties are available:
