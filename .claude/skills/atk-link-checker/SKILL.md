---
name: atk-link-checker
---

# ATK VuePress 链接检查与修复工具

## 核心原则

1. **逐条核验，禁止盲修**：每个疑似问题都必须查看所在文件、行号、上下文和目标候选，不能只靠脚本结果批量替换。
2. **先判断是否真错**：VuePress 支持多种合法链接形式，不能因为普通文件路径解析失败就认定为坏链。
3. **只修确定性问题**：只有目标唯一、语义匹配、链接形式明确错误时才修改。
4. **不确定就不改**：无法确认正确目标的链接保留原样，最终只报告这些链接并给出建议。
5. **仅手动触发**：此技能不根据上下文自动激活。
6. **不主动构建**：完成修复后不要运行 `npm run build:standalone`，用户自行验证。
7. **最终报告保持精简**：最终只告诉用户仍不确定、需要用户确认的链接，不输出已修复清单或无问题统计。

## 检查范围

默认检查以下 Markdown 文档目录里的所有`.md`文档：

- `src/en`
- `src/zh`

额外检查以下目录和配置文件中的链接：

### `src/.vuepress/components` 目录（所有文件）

检查该目录下所有 Vue 组件（`.vue`）、JavaScript（`.js`/`.mjs`）和 Markdown（`.md`）文件中的链接，包括但不限于：

- `RouteLink :to` / `router-link to` 的目标路径
- `<a href>` / `:href` 的目标路径
- `<img src>` / `:src` 的图片路径
- `import.meta.glob` / `import.meta.globEager` 的扫描模式（确认模式能匹配到实际文件）
- 组件代码中硬编码的文件路径引用（如 `path.join`、`require`、`import` 引用项目内文件）
- Markdown 文件中的链接（如 `ConnectCommandSummary/atk-command-syntax-highlighting.md`）

其中重点文件：

- `OverView/OverView.vue` — 模块数据配置中的 `link` 字段
- `ConnectCommandSummary/ConnectCommandSummary.vue` — `import.meta.glob` 扫描路径，以及提取后生成的 `RouteLink` 目标路径
- `CatalogCard/CatalogCard.vue` — `import.meta.glob` 扫描路径，动态生成的 `RouteLink :to` 和 `<img :src>`；其中 `resolveImagePath` 负责把文档第一张图片（默认封面）归一化为站点路径，需正确解析 `../`、`./`，en 文档图片指向 `../../zh/...` 时才能命中 `imageUrlMap`

### `scripts/` 目录

检查该目录下所有脚本文件（`.js`/`.cjs`/`.mjs`）中的文件路径引用，确保以下类型路径能正确解析到实际存在的文件：

- `require()` / `import` 引用项目内相对路径
- `fs.readFileSync()` / `fs.readdirSync()` 等文件操作的目标路径
- `path.join()` / `path.resolve()` 引用的项目内文件或目录路径
- 脚本注释或配置中明确指定的文档目录路径

### `.vuepress/redirect.ts`

- 重定向源地址和目标地址

除非某个文件是上述范围内链接的目标候选，否则不要主动扫描其他 Markdown 目录。

## VuePress 合法形式识别

检查前必须考虑以下形式可能都是合法的：

- Markdown 文件路径：`foo.md`、`./foo.md`、`../foo.md`
- VuePress 页面路由：`foo.html`、`./foo.html`、`/foo.html`
- 目录路由：`foo/`、`./foo/`、`/foo/`
- 省略扩展名：`foo`、`./foo`、`../foo`
- 锚点：`#anchor`、`./foo.md#anchor`、`/foo/#anchor`
- 中文路径、空格路径、URL 编码路径
- VuePress 标题自动生成的锚点
- Markdown-it / VuePress 插件生成或支持的自定义锚点
- 静态资源路径、相对图片路径、绝对 public 路径
- **含括号的 URL**：markdown-it（VuePress 底层 Markdown 解析器）支持 URL 中平衡括号的解析。例如 `![alt](./path/file(1).jpg)` 是合法链接——解析器会正确匹配括号对，不会将 URL 内部的 `(1)` 截断。脚本用正则 `[^)]+` 提取 URL 时会误截断，但这**不是真实断链**，必须结合文件系统验证而非仅依赖正则结果

只有在确认 VuePress 不能解析该链接，或构建明确报错，且正确目标能够唯一确定时，才进行修复。

### 多语言站点（/zh/ 与 /en/）的额外规则

本项目通过 `.vuepress/config.ts` 配置了 `/zh/` 与 `/en/` 两个 locale，多语言下页面实际路由为 `/zh/...`、`/en/...`。以下链接形式虽然「看起来合法」，实际是坏链，检查时必须特别注意：

- **不带语言前缀的绝对路由链接**（如 `/二次开发教程/`）：不会落到正确的 locale，是坏链，应改为相对路径（如 `../../二次开发教程/`）。单语言站点里这种绝对路径可用，多语言下必须改用相对路径。
- **frontmatter `thumbnail` 的绝对路径**（如 `/02-案例教程/media/...`）：`CatalogCard` 组件通过 `imageUrlMap`（键为 `/zh/...`、`/en/...`）解析缩略图，不带语言前缀命中不到，图片渲染不出来。应改为 `/zh/02-案例教程/...`；en 文档复用中文图片，同样指向 `/zh/...`。
- **en 文档复用 zh 图片**：en 侧不保留重复 media 文件，图片链接统一指向 `../../zh/02-案例教程/...`。

## 工作流程

### 第1步：确认项目结构

- 确认当前仓库是 VuePress 文档项目。
- 查看 `.vuepress/config.ts`，理解 base、路由、主题或插件配置是否会影响链接解析。
- 查看 `package.json`，确认构建命令可用。
- 确认上述检查范围目录和两个配置文件是否存在。

### 第2步：收集候选目标

- 收集检查范围内的 Markdown 文件。
- 收集仓库中的 Markdown 文件作为链接目标候选。
- 收集图片和静态资源文件作为图片路径候选。
- 对 Markdown 标题建立锚点候选，但不要完全依赖脚本生成规则；中文标题、自定义锚点和插件锚点必须谨慎判断。

### 第3步：扫描链接

检查以下链接来源：

- Markdown 普通链接：`[text](target)`
- Markdown 图片：`![alt](target)`
- HTML 图片：`<img src="target">`
- HTML 链接：`<a href="target">`
- VuePress/Vue 组件中明显的链接属性：如 `to`、`href`、`RouteLink :to`、`router-link to`
- `.vuepress/components/` 目录下所有 `.vue`/`.js`/`.mjs` 文件中的：
  - `RouteLink :to` 和 `router-link to` 的目标路径
  - `<a href>` / `:href` 的目标路径
  - `<img src>` / `:src` 的图片路径
  - `import.meta.glob` / `import.meta.globEager` 的扫描模式（确认模式能匹配到实际文件）
  - 硬编码的文件路径引用
- `.vuepress/components/` 目录下的 `.md` 文件中的 Markdown 链接和文件路径引用
- `scripts/` 目录下所有 `.js`/`.cjs`/`.mjs` 文件中的：
  - `require()` / `import` 引用的项目内相对路径
  - `fs.readFileSync()` / `fs.readdirSync()` 等文件操作的目标路径
  - `path.join()` / `path.resolve()` 引用的项目内文件或目录路径
  - 注释或配置中明确指定的文档目录路径
- `.vuepress/redirect.ts` 中的重定向地址

**特殊处理 `<!--@include:...-->` 指令：**

Markdown 文件可能通过 `<!--@include:path/to/file.md-->` 嵌入其他文件的内容。VuePress 构建时会将 include 文件的内容直接复制到源文件中，include 文件内的相对链接**以 include 文件自身的位置为基准解析**，而非以包含它的源文件位置为基准。

检查步骤：
1. 扫描所有文件时，对检查范围内的每个 `.md` 文件，查找 `<!--@include:RELATIVE_PATH-->` 指令。
2. 解析 `RELATIVE_PATH` 得到实际的 include 文件路径。
3. **读取 include 文件内容**，从中提取所有 Markdown 链接 `[text](target)`。
4. 对 include 文件中的每个相对链接，**以 include 文件所在目录为基准**解析目标路径，验证目标是否存在。
5. 注意：同一个 include 文件可能被多个位于不同目录层级的源文件引用。如果 include 中的链接使用相对路径，必须确保从 include 文件自身位置出发能够正确解析到目标。如果 include 被不同深度的文件引用会导致相对路径失效，这是一个需要报告的真正问题。

跳过以下内容：

- 代码块中的示例链接，除非上下文表明它是真实文档链接。
- 纯占位符示例，如 `http://example.com`。
- `mailto:`、`tel:`、`javascript:` 等非文档导航链接。

### 第4步：逐条判断疑似问题

对每个疑似坏链，必须逐条完成以下检查：

1. 打开源文件并查看链接所在行及上下文。
2. 判断链接意图：跳转文档、跳转章节、引用图片、访问外部站点、VuePress 路由或历史重定向。
3. 判断原链接是否可能是 VuePress 支持的合法形式。
4. 查找目标候选，确认是否只有一个合理目标。
5. 对比链接文本、上下文标题、候选文件名和候选页面内容，确认语义是否匹配。
6. 如果修复会改变链接风格，确认是否必要；不要为了统一风格而改动本来有效的链接。
7. **`.md` 后缀与目录不匹配检查**：当链接以 `.md` 结尾时，必须确认目标以**文件**形式存在（而非仅存在同名目录）。如果只有 `xxx/` 目录存在而 `xxx.md` 文件不存在，该链接是断链——VuePress 不会将 `.md` 后缀链接自动解析到同名目录路由。此时应检查目录内是否有 `README.md` 或 `index.md`，如有则链接应改为 `xxx/`（目录路由）。
8. **子目录相对路径检查**：当源文件位于子目录中时，其相对链接以源文件所在目录为基准解析。如果目标在该目录下不存在，必须**向上搜索父目录**，检查目标是否实际位于父级或更上层目录。如果目标在父目录中找到且语义匹配，链接缺少 `../` 前缀，是断链。
9. **目录路由无需 README.md**：链接以 `/` 结尾指向目录路由时（如 `xxx/`），即使目标目录内没有 `README.md` 或 `index.md`，该链接也**不一定是断链**。vuepress-theme-hope 的侧边栏配置（如 `children: "structure"`）会自动为目录生成导航结构，目录路由仍然有效。不要因为目录缺少索引文件就将其判定为坏链。

## 自动修复规则

可以自动修复的情况：

- 相对路径层级明显错误，且只有一个语义匹配的目标文件。
- 图片路径明显错误，且只有一个语义匹配的图片文件。
- 大小写不一致，且仓库中只有一个大小写匹配候选。
- `.md`、`.html`、目录路由之间的形式错误导致构建失败，且正确目标唯一。
- **`.md` 链接指向同名目录**：链接是 `xxx.md` 但只有 `xxx/` 目录存在（且目录内有 `README.md`），应改为 `xxx/`。
- **子目录文件缺少 `../` 前缀**：链接目标实际在父目录而非当前目录，应补充 `../` 前缀。
- 链接指向已重命名文件，且通过上下文和文件内容能确认唯一目标。
- **绝对路由链接改相对路径**：多语言站点下不带语言前缀的绝对路由（如 `/二次开发教程/`）是坏链，应改为相对路径（保留原目录路由 `/` 结尾风格）。
- **thumbnail 缩略图补语言前缀**：`thumbnail` 用绝对路径且不带 `/zh/`、`/en/` 前缀时，应补全为 `/zh/...`（en 文档复用中文图片，同样用 `/zh/...`）。

自动修复时必须遵守：

- 每次修改前确认所在上下文和目标语义。
- 优先保持该文件已有链接风格。
- 使用最小改动，不做无关格式化。
- 不使用全局批量替换来代替逐条确认。

## 不自动修复规则

以下情况只记录为”不确定链接”，不修改：

- 多个候选目标都可能正确。
- 候选目标存在，但语义和链接文本不明显匹配。
- 锚点可能由中文标题、自定义 `{#id}`、插件或运行时组件生成，无法静态确认。
- `.vuepress/redirect.ts` 中源地址是历史兼容路径，无法判断是否应该保留。
- 外部链接不可访问、超时、重定向异常或需要登录。
- VuePress 可能支持该写法，但静态检查无法完全确认。
- 修复会影响大量文件或需要统一迁移链接风格。
- **URL 含括号**：markdown-it 支持平衡括号解析，含 `(n)` 的 URL 是合法的。不要将括号 URL-encoded（如 `%28` / `%29`），这反而可能因文件系统差异导致链接失效。
- **目录路由无索引文件**：链接以 `/` 结尾指向目录时，即使目录内没有 `README.md`/`index.md`，也不一定是断链。vuepress-theme-hope 的侧边栏可自动为该目录生成导航。

## 最终报告格式

最终只输出仍需用户确认的不确定链接。不要输出已自动修复链接清单，不要输出无问题统计。

如果没有不确定链接，简短说明：

```markdown
链接检查完成。没有需要你确认的不确定链接。
```

如果存在不确定链接，使用以下格式：

```markdown
链接检查完成。以下链接需要你确认：

1. `文件路径:行号`
   - 原链接：`...`
   - 不确定原因：...
   - 建议：...
```

## 禁止事项

- 禁止用一次全局替换批量改完所有疑似链接。
- 禁止只根据文件名相似度自动修复。
- 禁止为了让构建通过把链接随意改成某个存在路径。
- 禁止将 VuePress 支持的合法链接形式误报为错误。
- **禁止将含括号的 URL 进行 URL 编码**（如 `file(1).jpg` → `file%281%29.jpg`）。markdown-it 原生支持平衡括号解析，URL 编码反而会因文件系统差异导致链接失效。
- 禁止在最终报告中输出冗长的已修复列表，除非用户明确要求。
